...
export default function CustomerEarningsPage() {
  ...
  const load = () => {
    ...
    Promise.all([payoutsApi.earnings(), payoutsApi.history()])
      .then(([earnRes, histRes]) => {
        setEarnings(earnRes.earnings ?? earnRes);
  ...
  const min = earnings?.min_threshold ?? 500;
  ...
  return (
    <div className="space-y-6">
      <PageHero
        title="Earnings"
        description="View your earnings from private listings. A small commission is deducted; once you reach the minimum threshold, you can request a payout to your bank account."
        illustration="earnings"
        guide="Tip: Earnings appear here after buyers pay for your listings. Request payout when your balance meets the minimum; payouts are processed to your bank account."
      />
      ...
      <div className="bg-white rounded-xl border border-gray-200/80 p-6 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <input
            ...
            className="w-full max-w-[200px] px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#1790d7] focus:border-[#1790d7]"
          />
          <button
            type="button"
            onClick={() => setAmountInput(String(net))}
            className="text-sm text-[#1790d7] hover:text-[#1277b8] font-medium"
          >
        ...
        <button
          ...
          className="px-6 py-3 bg-[#1790d7] hover:bg-[#1277b8] text-white font-semibold rounded-xl transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
